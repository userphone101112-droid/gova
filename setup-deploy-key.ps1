#!/usr/bin/env pwsh
# Setup Deploy Key for gova Project
...
$sshLocalPath = "$env:USERPROFILE\.ssh_local\gova"

function Show-Help {
    Write-Host @"
Deploy Key Setup Script for gova
...
function Setup-DeployKey {
    ...
    $sshConfig = @"
Host github.com-gova
    HostName github.com
    User git
    IdentityFile $sshLocalPath\.git_deploy_key
    IdentitiesOnly yes
"@
    $sshConfig | Out-File -FilePath "$projectRoot\.ssh_config" -Encoding utf8
    ...
    if ($currentRepo -like "*http*") {
        ...
        Write-Host "New URL: git@github.com-gova:userphone101112-droid/gova.git"
        git remote set-url origin "git@github.com-gova:userphone101112-droid/gova.git"
        Write-Host "Remote updated" -ForegroundColor Green
    }

    Write-Host "`nSetup completed!" -ForegroundColor Green
    Write-Host "Next step: Add the public key to GitHub Deploy Keys:"
    Write-Host "  https://github.com/userphone101112-droid/gova/settings/keys" -ForegroundColor Cyan
    Write-Host "  Enable 'Allow write access'" -ForegroundColor Yellow
}

function Test-DeployKey {
    Write-Host "Testing SSH connection..." -ForegroundColor Cyan
    Write-Host "Attempting connection..."
    ssh -T git@github.com-gova 2>&1
